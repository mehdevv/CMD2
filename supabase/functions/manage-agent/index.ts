...
Deno.serve(async req => {
  if (req.method === 'OPTIONS') {
    // Explicit 2xx + full CORS so browser preflight succeeds (some stacks treat default status as non-OK).
  ...
  if (meErr || !me?.org_id) {
    return json({ error: 'No profile or organization' }, 403);
  }
  ...
  if (orgErr || !org?.slug) {
    return json({ error: 'Set your business address (slug) on the team page before adding agents.' }, 400);
  }
  ...
  if (body.action === 'create') {
    ...
    if (!HANDLE_RE.test(h)) {
      return json({ error: 'Handle must be 3–40 chars: lowercase letters, digits, . _ -' }, 400);
    }
